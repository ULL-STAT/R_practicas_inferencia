...
#================================
# (B) GENERAR MUESTRAS DE VALORES ALEATORIOS
#================================
...
set.seed(149653)   # la semilla aleatoria se utiliza para que a todos nos salgan las mismas simulaciones
n <- 20
x1 <- runif(n)                 # muestra de U(0,1]
x2<-round(26712*x1, digits=0)  # muestra de U(0,26712]
x3<-sort(x2)
print(x3)
...
y<-sample(1:26712, 20, replace = FALSE, prob = NULL)
print(sort(y))
...
qqline(x, distribution = qnorm,   prob = c(0.25, 0.75), col = 2,lwd=3, qtype=9)  #col= color de la línea, 0.25 y 0.75 son valores por defecto
...
#Comprobar para mu=10, sigma^2=100, con n=50, haciendo 10000 réplicas de una suma X1+ ...+ Xn
set.seed(149653)
