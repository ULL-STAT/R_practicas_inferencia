...
print(anova1F) 
med1F<-tapply(desgaste, tipo, mean)    # medias de la var. respuesta para cada nivel del factor"
print(med1F)
sd1F<-tapply(desgaste, tipo, sd)       # desviaciones tipicas de la var. respuesta para cada nivel del factor"
print(sd1F)
# gráfico de las medias de la var. respuesta en cada nivel del factor
plot(med1F,type='o',pch=19, 
     col="red",ylim=c(1.8,3.0),
     xlab="tipo de madera",ylab="media del desgaste",
     main="MEDIAS",cex.main=1.0)
text(1:5, med1F-0.1, labels=names(med1F))



bartlett.test(desgaste~tipo)          # contraste de homogeneidad de varianzas
boxplot(desgaste~tipo)                # comparar la varianza de desgaste frente a cada tipo de madera

library(ggplot2)
library(dplyr)
madera %>% ggplot(aes(sample = desgaste)) +
  geom_qq() + geom_qq_line() +
  facet_wrap(~tipo, scales = "free_y") # comparar la normalidad de la muestra para cada tipo de madera


aov1F<-aov(desgaste ~ tipo,  data=madera)                    #modelo lineal para comparaciones múltiples 
library(stats)
medtu<-TukeyHSD(aov1F)                                        # test de comparaciones de Tukey HSD
medtu
plot(medtu)
pairwise.t.test(desgaste,tipo,p.adjust.method="bonferroni")  # ajuste de Bonferroni
...
text(1:5, med1f-10, labels=names(med1f))
...
text(1:8, med1b-10, labels=names(med1b))
